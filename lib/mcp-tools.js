...
export function registerTools(server, bridge, artboards, project) {
  ...
  /**
   * 发送命令到 Webview（有客户端时实时，无客户端时立即入队，不阻塞）
   */
  async function sendVisualCommand(type, payload = {}) {
    // 检查是否有真正的 Webview 客户端（本地或远程）
    if (!bridge.hasActiveWebviewClient) {
      // 没有 Webview — 直接入队，立刻返回
      bridge.enqueueCommand(type, payload);
      return { queued: true, message: 'Command queued — will render when canvas panel opens' };
    }
    try {
      ...
      // 发送失败 — 入队等待
      bridge.enqueueCommand(type, payload);
      return { queued: true, message: `Command queued (${err.message})` };
    }
  }
