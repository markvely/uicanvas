...
export function registerTools(server, bridge, artboards, project) {
  ...
  server.tool('open_canvas',
    ...
    { _ping: z.boolean().optional().describe('Dummy parameter for schema validation') },
    async () => {
      ...
      if (Date.now() - startTime >= maxWaitMs) {
        return {
          content: [{ type: 'text', text: JSON.stringify({
            ...
            message: '❌ Could not connect to UICanvas HTTP server. ⚠️ If you are running inside Cursor, Windsurf, or Claude Desktop, the UICanvas extension REQUIRES a VS Code / Antigravity editor window to be OPEN. Please tell the user to open VS Code/Antigravity and keep it active to render the UI. (Otherwise try Reload Window)',
          }, null, 2) }],
          isError: true,
        };
      }
      ...
      return {
        content: [{ type: 'text', text: JSON.stringify({
          ...
          message: '❌ Canvas panel did not connect within 30 seconds. ⚠️ If using an external AI (Cursor/Claude Desktop), please tell the user to open the VS Code/Antigravity editor so the canvas can render. (Otherwise try Reload Window).',
        }, null, 2) }],
        isError: true,
      };
    }
...
5. Now call create_artboard freely for your UI pages`,
    {
      ...
      designTokens: z.object({
        colors: z.string().optional().describe('JSON string of color overrides, e.g. {"primary": "#FF6B6B"}'),
        fonts: z.string().optional().describe('JSON string of font overrides, e.g. {"heading": "Inter, sans-serif"}'),
      }).optional().describe('Optional custom design tokens to override defaults'),
    },
    async ({ projectName, fileName, designTokens }) => {
      if (designTokens) {
        if (typeof designTokens.colors === 'string') {
          try { designTokens.colors = JSON.parse(designTokens.colors); } catch(e){ delete designTokens.colors; }
        }
        if (typeof designTokens.fonts === 'string') {
          try { designTokens.fonts = JSON.parse(designTokens.fonts); } catch(e){ delete designTokens.fonts; }
        }
      }
      // 初始化项目状态
...
💡 RECOMMENDED WORKFLOW: Call init_project first to establish design specs (colors, fonts, spacing) before creating page artboards.`,
    {
      ...
      styles: z.string().optional().describe('CSS styles object as a JSON string'),
    },
    async ({ name, width, height, project: proj, page, state, styles }) => {
      if (typeof styles === 'string') {
        try { styles = JSON.parse(styles); } catch(e){ return { content: [{ type: 'text', text: 'Error: invalid JSON string for styles' }], isError: true }; }
      }
      // 建议性工作流检查
  ...
  server.tool('update_styles',
    'Update styles on one or more nodes.',
    {
      updates: z.array(z.object({
        nodeIds: z.array(z.string()),
        styles: z.string().describe('CSS styles object as a JSON string'),
      })),
    },
    async ({ updates }) => {
      ...
      for (const update of updates) {
        if (typeof update.styles === 'string') {
          try { update.styles = JSON.parse(update.styles); } catch(e){ return { content: [{ type: 'text', text: `Error: invalid JSON string for styles in update_styles: ${update.styles}` }], isError: true }; }
        }
      }
