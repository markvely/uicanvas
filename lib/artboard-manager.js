...
export class ArtboardManager {
  ...
  /**
   * 创建画板
   * @param {{ name: string, width: number, height: number, styles?: object }} opts
   * @returns {ArtboardMeta}
   */
  create({ name, width, height, styles = {} }) {
    const id = `ab-${randomUUID().slice(0, 8)}`;
    const left = this.nextLeft();
    const meta = { id, name, width, height, left, top: 0, styles, children: [] };
  ...
  /** 添加子节点记录 */
  addChild(artboardId, nodeId) {
    const ab = this.artboards.get(artboardId);
    if (ab) ab.children.push(nodeId);
  }

  /** 根据节点 ID 找到所属画板 */
  findArtboardByNode(nodeId) {
    for (const ab of this.artboards.values()) {
      if (ab.children.includes(nodeId)) return ab;
    }
    return null;
  }
